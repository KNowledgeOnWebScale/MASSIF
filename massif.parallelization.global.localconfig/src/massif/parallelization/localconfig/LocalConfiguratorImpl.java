...
/**
 * Configures the number of duplicate services in a local osgi container.
 * A number of active services can be indicated through the configuration manager.
 *  *param <instances> indicates the number of active instances.
 *  *param <factoryPID> indicates the servicec.pid of the to be duplicated service
 *
 * @author pbonte
 *
 */
@Component(immediate = true, property = {"service.pid=massif.parallelization.localconfig.LocalConfiguratorImpl","massif.parallel.node=remote","aiolos.unique=true" })
public class LocalConfiguratorImpl implements LocalConfigurator, ManagedService {
	...
	@Reference(unbind="unbindDeploymentManager")
	public void bindDeploymentManager(DeploymentManager deployManager){
		logger.info("Binded Deployment Manager");
		this.deploymentManager = deployManager;
		

	}
	public void unbindDeploymentManager(DeploymentManager deployManager){
		if(deployManager == deploymentManager){
	...
	private void startComponent(String componentName){
		try {			
			ComponentInfo info = deploymentManager.startComponent(componentName);
		} catch (DeploymentException e) {
			logger.info("Unable to deploy new component <"+componentName+">." ,e);
		}
	}
	private void stopComponent(String componentName){
		try {
			for(ComponentInfo cinfo:deploymentManager.getComponents()){
				if(cinfo.getName().equals(componentName)){					
					deploymentManager.stopComponent(cinfo);
					logger.info("Stopping component <"+componentName+">.");
					return;
				}
					
			}
			logger.error("Was not able to stop component <"+componentName+">.");
			
		} catch (DeploymentException e) {
			logger.info("Unable to stop new component <"+componentName+">.");
		}
	}
	private boolean isServiceActive(String pid, String clazz){
		try {
			ServiceReference[] refs = context.getAllServiceReferences(clazz, "(&(service.pid="+pid+")(aiolos.proxy.local=true))");
			if(refs == null){
				refs = context.getAllServiceReferences(clazz, "(&(component.name="+pid+")(aiolos.proxy.local=true))");
			}
			if(refs!=null){
	...
	public void updated(Dictionary<String, ?> properties) throws ConfigurationException {
		...
		String componentName = (String) properties.get("componentName");
		try {
			...
			if(!isActive){
				if(componentName == null){
					logger.error("No config found for <"+factoryPid+">. Can not deploy, missing componentName");
				}else{				
					startComponent(componentName);
					logger.info("No config found for <"+factoryPid+">. Deploying component <"+componentName+">.");
				}
				isActive  = isServiceActive(factoryPid, clazz);
			}
			...
			//if null, there are no configs active and we should not create a factory for the pid
			if (originalConfig != null && updatedInstances != null ) {
				int instances = 0; //if no configs exists, no factory services are registered
				...
				if (updatedInstancesInt > instances) {
					...
					for (int i = 0; i < numberOfnewInstances; i++) {
						...
						if(originalID != null){
							...
							logger.error("New service not synched, no original ID has been passed");
						}
					}
					...
					for (int i = instances; i > updatedInstancesInt && i > 0; i--) {
						//retrieve last configs from the registered configurations
						Configuration stopConfig = currentConfigs[i-1];
						//by calling delete on a managed factory service, the deactivate method is invoked
						stopConfig.delete();
					}
				}
	...
	public void updateService(String clazz, String original, String duplicate){
		try {
			...
			ServiceReference[] originalRef = context.getAllServiceReferences(clazz, "("+ORIGINAL_IDENTIFIER+"="+original+")");
			...
			ServiceTracker tracker = new ServiceTracker<DuplicateService,DuplicateService>(context, context.createFilter("("+IDNAME+"="+duplicate+")"),dTracker);
