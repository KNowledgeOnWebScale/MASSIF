...
public class ProtegeJavaMapping {
   ...
   // protected final Logger LOGGER = Logger.getLogger(this.getClass());
    ...
    public void initialize(OWLOntology ontology, CodeGenerationInference inference, ContextModelListener decisionEngine) {
    ...
    public Class<?> getJavaImplementationFromOwlClass(OWLClass cls) {
        JavaAndOwlBean bean = protegeMap.get(cls);
        return bean != null ? bean.getJavaImplementation() : null;
    }
    ...
    private <X> X constructImplementation(Class<? extends X> implType, IRI id) {
        try {
            Constructor<? extends X> con = implType.getConstructor(new Class[] { OWLOntology.class, IRI.class,CodeGenerationInference.class, ProtegeJavaMapping.class,ContextModelListener.class});
            X x=con.newInstance(new Object[] { ontology, id,inference, this,decisionEngine });
            if (decisionEngine != null){
    			decisionEngine.contextObjectCreated(x);
    			//LOGGER.info("Update object type - "+x.toString());
            }
