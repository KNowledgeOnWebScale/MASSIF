...
public class WrappedIndividualImpl implements WrappedIndividual {
	...
	public WrappedIndividualImpl(OWLOntology owlOntology,
	...
	public void assertOwlType(OWLClassExpression type) {
		...
		manager.addAxiom(owlOntology,
				factory.getOWLClassAssertionAxiom(type, owlIndividual));
	}
	 ...
	 */
	public void delete() {
		OWLEntityRemover remover = new OWLEntityRemover(getOwlOntology()
				.getOWLOntologyManager(),
				Collections.singleton(getOwlOntology()));
		owlIndividual.accept(remover);
		getOwlOntology().getOWLOntologyManager().applyChanges(
				remover.getChanges());

		if(decisionEngine!=null){
		decisionEngine.contextObjectRemoved(this);
		//LOGGER.info("Remove object - "+this.getOwlIndividual().getIRI());
		}
		}
	...
	public boolean equals(Object obj) {
		...
		return other.getOwlOntology().equals(owlOntology)
				&& other.getOwlIndividual().equals(owlIndividual);
	}
	...
	private void printTypes(StringBuffer sb, ShortFormProvider provider) {
		...
		for (OWLClass ce : inference.getTypes(owlIndividual)) {
			if (!ce.isAnonymous()) {
				types.add(ce);
			}
		}
	...
	private void printObjectPropertyValues(StringBuffer sb,
			ShortFormProvider provider) {
		Map<OWLObjectPropertyExpression, Set<OWLIndividual>> valueMap = new TreeMap<OWLObjectPropertyExpression, Set<OWLIndividual>>(
				owlIndividual.getObjectPropertyValues(owlOntology));
		for (Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : valueMap
				.entrySet()) {
			...
			if (!pe.isAnonymous()) {
				...
				for (OWLIndividual value : values) {
					if (!value.isAnonymous()) {
						...
						sb.append(provider.getShortForm(value
	...
	private void printDataPropertyValues(StringBuffer sb,
			ShortFormProvider provider) {
		Map<OWLDataPropertyExpression, Set<OWLLiteral>> valueMap = new TreeMap<OWLDataPropertyExpression, Set<OWLLiteral>>(
				owlIndividual.getDataPropertyValues(owlOntology));
		for (Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : valueMap
				.entrySet()) {
